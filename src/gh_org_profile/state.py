...
def save_state(output_dir: Path, org: str, repos: list) -> None:
    state = RunState(
        ...
        repo_last_activity={
            r.name: r.pushed_at.isoformat() if r.pushed_at else None
            for r in repos
        },
    )
    p = output_dir / f"{org}_state.json"
...
def classify_repos(repos: list, prev_state: RunState | None, dormancy_days: int = 90) -> tuple[list, list]:
    if prev_state is None:
        cutoff = datetime.now(timezone.utc) - timedelta(days=dormancy_days)
        active = [r for r in repos if r.pushed_at and r.pushed_at.replace(tzinfo=timezone.utc) > cutoff]
        dormant = [r for r in repos if not r.pushed_at or r.pushed_at.replace(tzinfo=timezone.utc) <= cutoff]
