...
class TestLoad:
    ...
    def test_returns_dict_when_file_exists(self, tmp_path):
        p = path(tmp_path, "myorg")
        p.write_text(json.dumps({"org": "myorg", "last_stage": "commits"}))
        result = load(tmp_path, "myorg")
        assert result == {"org": "myorg", "last_stage": "commits"}
...
class TestSave:
    def test_creates_checkpoint_file(self, tmp_path):
        save(tmp_path, "myorg", "commits", commit_data={"repo-a": {}})
        assert path(tmp_path, "myorg").exists()

    def test_sets_last_stage(self, tmp_path):
        save(tmp_path, "myorg", "quality")
        result = load(tmp_path, "myorg")
        assert result["last_stage"] == "quality"

    def test_stores_extra_data(self, tmp_path):
        save(tmp_path, "myorg", "commits", commit_data={"repo-a": {"total_commits": 5}})
        result = load(tmp_path, "myorg")
        assert result["commit_data"] == {"repo-a": {"total_commits": 5}}

    def test_merges_with_existing_checkpoint(self, tmp_path):
        save(tmp_path, "myorg", "commits", commit_data={"repo-a": {}})
        save(tmp_path, "myorg", "quality", quality_data={"repo-a": {}})
        result = load(tmp_path, "myorg")
    ...
    def test_later_save_advances_last_stage(self, tmp_path):
        save(tmp_path, "myorg", "commits")
        save(tmp_path, "myorg", "quality")
        assert load(tmp_path, "myorg")["last_stage"] == "quality"
    ...
    def test_save_does_not_overwrite_existing_key_with_none(self, tmp_path):
        save(tmp_path, "myorg", "commits", commit_data={"repo-a": {}})
        # Second save omits commit_data — existing value should be preserved
        save(tmp_path, "myorg", "quality", quality_data={"repo-a": {}})
        result = load(tmp_path, "myorg")
        assert result["commit_data"] == {"repo-a": {}}
...
class TestIsComplete:
    ...
    def test_full_stage_ordering(self):
        stages = ["repo_metadata", "topics", "commits", "quality", "connections", "readme", "contributors"]
    ...
    def test_contributors_complete_implies_all_complete(self):
        ...
        for stage in ["repo_metadata", "topics", "commits", "quality", "connections", "readme", "contributors"]:
    ...
    def test_repo_metadata_complete_implies_only_itself_complete(self):
        ...
        for stage in ["topics", "commits", "quality", "connections", "readme", "contributors"]:
...
class TestSaveLoadRoundTrip:
    def test_full_round_trip(self, tmp_path):
        ...
        save(tmp_path, "myorg", "quality", commit_data=commit_data, quality_data=quality_data)
        result = load(tmp_path, "myorg")
        assert result["last_stage"] == "quality"
    ...
    def test_resume_skips_completed_stages(self, tmp_path):
        ...
        assert is_complete(ckpt, "commits") is True
        assert is_complete(ckpt, "quality") is True
        assert is_complete(ckpt, "connections") is False
